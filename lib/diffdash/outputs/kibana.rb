    ...
    class Kibana < Base
      ...
      def build_visualizations(signal_bundle)
        ...
        signal_bundle.metrics.each_with_index do |signal, idx|
          ...
          when :counter
            visualizations << counter_visualization(signal, offset + idx)
          when :gauge
            visualizations << gauge_visualization(signal, offset + idx)
          when :histogram
            visualizations << histogram_visualization(signal, offset + idx)
          else
            visualizations << counter_visualization(signal, offset + idx)
          end
        end
