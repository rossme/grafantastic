...
RSpec.describe Diffdash::AST::Visitor do
  ...
  describe "#process" do
    ...
    context "with Loggy module" do
      it "detects log(...) calls in classes with Loggy::ClassLogger" do
        source = <<~RUBY
          class PaymentProcessor
      ...
      it "detects log(...) calls in classes with Loggy::InstanceLogger" do
        source = <<~RUBY
          class PaymentProcessor
        ...
        expect(visitor.log_calls.size).to eq(1)
        expect(visitor.log_calls.first[:level]).to eq("error")
        expect(visitor.log_calls.first[:event_name]).to eq("payment_failed")
      end

      it "detects log(...) calls with extend Loggy::ClassLogger" do
        source = <<~RUBY
          class PaymentProcessor
      ...
      it "handles log(...) with default info level" do
        source = <<~RUBY
          class PaymentProcessor
            include Loggy::ClassLogger
        ...
        expect(visitor.log_calls.size).to eq(1)
        expect(visitor.log_calls.first[:level]).to eq("info")
        expect(visitor.log_calls.first[:event_name]).to eq("payment_completed")
      end
      ...
      it "supports all log levels" do
        ...
        expect(visitor.log_calls.size).to eq(5)
        levels = visitor.log_calls.map { |l| l[:level] }
        expect(levels).to eq(%w[debug info warn error fatal])
      end

      it "does not detect log(...) in classes without Loggy modules" do
        source = <<~RUBY
          class PaymentProcessor
            def process
              log("this should not be detected")
            end
          end
        RUBY
        parse_and_visit(source)

        expect(visitor.log_calls).to be_empty
      end

      it "works alongside standard logger.info calls" do
        source = <<~RUBY
          class PaymentProcessor
            include Loggy::ClassLogger

            def process
              log(:info, "using_loggy")
              logger.info "using_standard_logger"
            end
          end
        RUBY
        parse_and_visit(source)

        expect(visitor.log_calls.size).to eq(2)
        event_names = visitor.log_calls.map { |l| l[:event_name] }
        expect(event_names).to include("using_loggy", "using_standard_logger")
      end

      it "handles Loggy in nested classes" do
        source = <<~RUBY
          module Services
            class PaymentProcessor
              include Loggy::InstanceLogger
        ...
        expect(visitor.log_calls.size).to eq(1)
        expect(visitor.log_calls.first[:defining_class]).to eq("Services::PaymentProcessor")
      end
    end
  end
end
