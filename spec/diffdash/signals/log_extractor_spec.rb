...
RSpec.describe Diffdash::Signals::LogExtractor do
  ...
  describe '.extract' do
    it 'creates Signal objects from log calls' do
      visitor = create_visitor_with_logs([
                                           { level: 'info', event_name: 'payment_processed',
    ...
    it 'preserves log level in metadata' do
      visitor = create_visitor_with_logs([
                                           { level: 'error', event_name: 'payment_failed',
                                             defining_class: 'PaymentProcessor', line: 10 }
                                         ])
    ...
    it 'generates fallback name when event_name is nil' do
      visitor = create_visitor_with_logs([
                                           { level: 'info', event_name: nil, defining_class: 'PaymentProcessor',
                                             line: 10 }
                                         ])
