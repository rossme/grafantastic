  ...
  describe '.extract' do
    it 'creates Signal objects from metric calls' do
      visitor = create_visitor_with_metrics([
                                              { name: 'payments_total', metric_type: :counter,
                                                defining_class: 'PaymentProcessor', line: 10 }
                                            ])
    ...
    it 'preserves line number in metadata' do
      visitor = create_visitor_with_metrics([
                                              { name: 'test_metric', metric_type: :counter, defining_class: 'Test',
                                                line: 42 }
                                            ])
    ...
    it 'skips metric calls with nil name' do
      visitor = create_visitor_with_metrics([
                                              ...
                                              { name: 'valid_metric', metric_type: :counter, defining_class: 'Test',
                                                line: 20 }
                                            ])
    ...
    it 'extracts multiple metric signals' do
      visitor = create_visitor_with_metrics([
                                              { name: 'requests_total', metric_type: :counter, defining_class: 'Test',
                                                line: 1 },
                                              { name: 'request_duration', metric_type: :histogram,
                                                defining_class: 'Test', line: 2 },
                                              { name: 'queue_size', metric_type: :gauge, defining_class: 'Test',
                                                line: 3 }
                                            ])
